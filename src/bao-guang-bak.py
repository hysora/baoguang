...
def open_login_page(platform: str):
    """
    打开对应平台的浏览器并跳转到首页供用户登录。
    由 UI 登录按钮调用，浏览器保持不关闭。
    """
    _PLATFORM_HOME = {
        "x":         "https://x.com",
    ...
    try:
        if platform == "tiktok":
            _, _, ctx = _ensure_tiktok()
        else:
            _, ctx = _ensure_context(platform)
        page = _get_or_new_page(ctx)
        print(f"[{platform.upper()}] 正在跳转 {url} ...")
        safe_goto(page, url)
        print(f"[{platform.upper()}] 浏览器已打开 {url}，请完成登录。")
        # 保持线程存活，防止 playwright context 因线程退出而失效
        while _is_context_alive(ctx):
            time.sleep(2)
    except Exception as e:
        print(f"[{platform.upper()}] 打开登录页失败: {e}")
